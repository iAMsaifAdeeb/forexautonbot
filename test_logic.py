...
# Live config defaults to 10 PIPS (V25).
check("live default entry_mode is 10_PIPS",
      LIVE_CONFIG.get("entry_mode") == "10_PIPS")
check("live active_strategies includes 10_PIPS",
      "10_PIPS" in (LIVE_CONFIG.get("active_strategies") or []))
check("live basket disabled for grid engines",
...
check("catalog has 10 strategies", len(STRATEGIES) == 10)
check("only 10_PIPS is implemented",
      is_implemented("10_PIPS")
